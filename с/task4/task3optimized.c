...
int main(int argc, char **args) {
    ...
    double current_eps;
    double new_max = 0;
    double current_max = max(myArr, arrW, arrH + 2);
    ...
    do {
        ...
        MPI_Recv(myArr[recvLine], arrW, MPI_DOUBLE, recvProc, MPI_ANY_TAG, MPI_COMM_WORLD, &status);

        for (int i = start; i < end; i += 2) {
            for (int j = 1; j < arrW - 1; j++) {
                myArr[i][j] = (myArr[i - 1][j] + myArr[i + 1][j] + myArr[i][j + 1] + myArr[i][j - 1]) / 4;
            }
        }

        new_max = max(myArr, arrW, arrH + 2);

        current_eps = fabs(new_max - current_max);
        current_max = new_max;
